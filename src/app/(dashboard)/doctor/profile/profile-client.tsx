...
import { Loader2, Stethoscope, Mail, Phone, Calendar, Award } from "lucide-react";
...
interface Props {
  doctor: { id: string; name: string; email: string; specialty?: string; phone?: string; createdAt: string };
  stats: { total: number; completed: number; records: number; patients: number };
}

export function DoctorProfileClient({ doctor, stats }: Props) {
  const [tab, setTab] = useState("profile");
  const [loading, setLoading] = useState(false);
  const [success, setSuccess] = useState("");
  const [profile, setProfile] = useState({ name: doctor.name, specialty: doctor.specialty || "General Medicine", phone: doctor.phone || "" });
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      ...
      {/* Profile header card */}
      <div className="bg-gradient-to-r from-[#0052FF] to-[#0080FF] rounded-2xl p-6 text-white">
        <div className="flex items-center gap-5">
          <div className="w-20 h-20 rounded-2xl bg-white/20 flex items-center justify-center text-3xl font-bold">
            {doctor.name[0]}
          </div>
          <div>
            <div className="text-2xl font-extrabold">Dr. {doctor.name}</div>
            <div className="text-blue-200 font-medium">{doctor.specialty || "General Medicine"}</div>
            <div className="text-blue-200 text-sm mt-1">{doctor.email}</div>
          </div>
        </div>
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <Input value={doctor.email} disabled className="mt-1 rounded-xl border-slate-200 h-10 bg-slate-50" />
              </div>
              <div>
